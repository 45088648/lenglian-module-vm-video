...
import com.beetech.module.adapter.ModuleBufPLvAdapter;
...
public class HistoryModuleBufFragment extends Fragment {

    private static final String TAG = HistoryModuleBufFragment.class.getSimpleName();

    private MyApplication myApp;
    private PagedList<ModuleBuf> mPagedList;
    private MyDataSource mDataSource;

    private class MyDataSource extends TiledDataSource<ModuleBuf> {
         ...
         */
        @Override
        public List<ModuleBuf> loadRange(int startPosition, int count) {
            Log.d("MyDataSource", "loadRange:" + startPosition + "," + count);
            List<ModuleBuf> list = loadData(startPosition, count);
            return list;
        }
     ...
     * @return
     */
    private List<ModuleBuf> loadData(int startPosition, int count) {
        List<ModuleBuf> dataList = new ArrayList();

        Message msg = new Message();
        msg.obj = "startPosition="+startPosition+", count = "+count;
        handlerToast.sendMessage(msg);

        dataList = myApp.moduleBufSDDao.queryAll(count, startPosition);
        return dataList;
    }
    ...
    private ModuleBufPLvAdapter rvAdapter;

    @ViewInject(R.id.rvModuleBufData)
    RecyclerView rvModuleBufData;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,  Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.history_module_buf_fragment,  container, false);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        init();
//        handler.postDelayed(task, 1000*60*10);
    }
    ...
    private void init() {
        registerForContextMenu(rvModuleBufData);
    }
    ...
    private void initData() {
        ...
        rvModuleBufData.setLayoutManager(mLayoutManager);
        rvAdapter = new ModuleBufPLvAdapter(mPagedList);
        rvModuleBufData.setAdapter(rvAdapter);

        rvModuleBufData.addOnScrollListener(new RecyclerView.OnScrollListener() {
            private int lastPos;
