...
import com.beetech.module.adapter.AppLogPLvAdapter;
...
public class HistoryAppLogFragment extends Fragment {

    private static final String TAG = HistoryModuleBufFragment.class.getSimpleName();
    private MyApplication myApp;

    private PagedList<AppLog> mPagedList;
    private MyDataSource mDataSource;

    private class MyDataSource extends TiledDataSource<AppLog> {
         ...
         */
        @Override
        public List<AppLog> loadRange(int startPosition, int count) {
            Log.d("MyDataSource", "loadRange:" + startPosition + "," + count);
            List<AppLog> list = loadData(startPosition, count);
            return list;
        }
     ...
     * @return
     */
    private List<AppLog> loadData(int startPosition, int count) {
        List<AppLog> dataList = new ArrayList();

        Message msg = new Message();
        msg.obj = "startPosition="+startPosition+", count = "+count;
        handlerToast.sendMessage(msg);

        dataList = myApp.appLogSDDao.queryAll(count, startPosition);
        return dataList;
    }
    ...
    private AppLogPLvAdapter rvAdapter;

    @ViewInject(R.id.rvAppLogData)
    RecyclerView rvAppLogData;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,  Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.history_app_log_fragment,  container, false);
...
//        handler.postDelayed(task, 1000*60*5);
    ...
    private void init() {
        registerForContextMenu(rvAppLogData);
    }
    ...
    private void initData() {
        ...
        rvAppLogData.setLayoutManager(mLayoutManager);
        rvAdapter = new AppLogPLvAdapter(mPagedList);
        rvAppLogData.setAdapter(rvAdapter);

        rvAppLogData.addOnScrollListener(new RecyclerView.OnScrollListener() {
            private int lastPos;
