...
public class WheelView extends View {
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			float textWidth = Double.valueOf(Math.ceil(Double.valueOf(Layout.getDesiredWidth("0", itemsPaint)))).floatValue();
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int) Math.ceil(Double.valueOf(Layout.getDesiredWidth(label, valuePaint)));
		}
